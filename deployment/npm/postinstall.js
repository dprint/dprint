...
if (fs.existsSync(targetExecutablePath)) {
  updatePkg(targetExecutablePath);
  process.exit(0);
}
...
const sourcePackagePath = (path.dirname(
  import.meta.resolve(`@dprint/${target}/package.json`, import.meta.url),
));
const sourceExecutablePath = path.join(sourcePackagePath, dprintFileName).slice(5);

if (!fs.existsSync(sourceExecutablePath)) {
  throw new Error(
    "Could not find executable for @dprint/" + target + " at " +
...
try {
  ...
  try {
    ...
    updatePkg(sourceExecutablePath, true);
    process.exit(0);
  } catch (_err) {
    // ignore
  }
...
function updatePkg(execPath, isSource = false) {
  const dprintPkg = path.join(__dirname, 'package.json');
  const pkgJSON = fs.readFileSync(dprintPkg, {encoding:'utf-8'});

  fs.writeFileSync(dprintPkg, pkgJSON.replace('bin.js', dprintFileName));
}
