...
RUN cmake -G Ninja \
        ...
        # inkwell (used by wasmer's LLVM backend) references the LLVMInitialize*
        # symbols for each of these targets, so they must all be built or linking
        # the loongarch64 dprint binary fails with undefined references
