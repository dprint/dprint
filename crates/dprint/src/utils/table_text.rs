use dprint_cli_core::logging::{LoggerTextItem, render_text_items_with_width};
...
impl TableText {
    ...
    pub fn get_logger_text_items(&self, indent: u16) -> Vec<LoggerTextItem> {
        ...
        for line in self.lines.iter() {
            ...
            if indent > 0 { text.push_str(&" ".repeat(indent as usize)); }
            text.push_str(&line);
...
pub fn get_table_text(items: Vec<(&str, &str)>) -> TableText {
    ...
    let lines = items.iter().map(|(key, value)| {
        let mut text = String::new();
        text.push_str(key);
