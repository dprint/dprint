...
//
// Vendored from deno_npmrc 0.8.0. Replace with the published crate once
// a release pins sys_traits >= 0.1.28. Keep changes minimal so this stays
    ...
    if let Ok(registry_url) = sys.env_var(env_var_name) {
      ...
      match Url::parse(&registry_url) {
        ...
        Err(_err) => {
          ...
          // a direct dep; invalid env var URLs silently fall through to the
          // fallback below
        }
      }
    }
