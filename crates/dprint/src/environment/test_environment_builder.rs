...
impl TestConfigFileBuilder {
    ...
    pub fn add_remote_process_plugin(&mut self) -> &mut Self {
        self.add_remote_process_plugin_with_checksum(
            &test_helpers::get_test_process_plugin_checksum(&self.environment),
        )
    }
...
impl TestEnvironmentBuilder {
    ...
    pub fn add_remote_wasm_plugin(&mut self) -> &mut Self {
        self.environment.add_remote_file("https://plugins.dprint.dev/test-plugin.wasm", test_helpers::WASM_PLUGIN_BYTES);
        self
    }
    ...
    pub fn add_local_wasm_plugin(&mut self) -> &mut Self {
        self.environment.write_file_bytes(&PathBuf::from("/plugins/test-plugin.wasm"), test_helpers::WASM_PLUGIN_BYTES).unwrap();
        self
    }
