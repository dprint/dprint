...
impl CliArgs {
  ...
  pub fn is_stdout_machine_readable(&self) -> bool {
    ...
    matches!(
      self.sub_command,
      SubCommand::StdInFmt(..) | SubCommand::EditorInfo | SubCommand::OutputResolvedConfig | SubCommand::OutputIncrementalState | SubCommand::Completions(..)
    )
  }
...
impl SubCommand {
  ...
  pub fn file_patterns(&self) -> Option<&FilePatternArgs> {
    match self {
      ...
      | SubCommand::OutputIncrementalState
      | SubCommand::Version
      | SubCommand::License
      | SubCommand::Help(_)
      | SubCommand::Lsp
...
fn inner_parse_args<TStdInReader: StdInReader>(args: Vec<String>, std_in_reader: TStdInReader) -> Result<CliArgs> {
  ...
  let sub_command = match matches.subcommand().unwrap() {
    ...
    ("output-incremental-state", _) => SubCommand::OutputIncrementalState,
...
EXAMPLES:
  ...
  Search for files using the specified file patterns:
    ...
    .subcommand(
      Command::new("output-incremental-state")
