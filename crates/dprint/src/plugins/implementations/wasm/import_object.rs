...
pub fn create_pools_import_object<TEnvironment: Environment>(
  ...
  let env = ImportObjectEnvironment::new(environment, plugins_collection, store.clone());
  ...
  (
    wasmer::imports! {
      "dprint" => {
        "host_clear_bytes" => Function::new_typed_with_env(&mut *store, &env, host_clear_bytes),
        "host_read_buffer" => Function::new_typed_with_env(&mut *store, &env, host_read_buffer),
        "host_write_buffer" => Function::new_typed_with_env(&mut *store, &env, host_write_buffer),
        "host_take_override_config" => Function::new_typed_with_env(&mut *store, &env, host_take_override_config),
        "host_take_file_path" => Function::new_typed_with_env(&mut *store, &env, host_take_file_path),
        "host_format" => Function::new_typed_with_env(&mut *store, &env, host_format),
        "host_get_formatted_text" => Function::new_typed_with_env(&mut *store, &env, host_get_formatted_text),
        "host_get_error_text" => Function::new_typed_with_env(&mut *store, &env, host_get_error_text),
      }
    },
    env,
  )
...
impl<TEnvironment: Environment> ImportObjectEnvironment<TEnvironment> {
  pub fn new(environment: TEnvironment, plugins_collection: Arc<PluginsCollection<TEnvironment>>, store: Arc<Mutex<Store>>) -> Self {
...
fn host_clear_bytes<TEnvironment: Environment>(mut env: FunctionEnvMut<ImportObjectEnvironment<TEnvironment>>, length: u32) {
  let env = env.data_mut();
  env.shared_bytes = vec![0; length as usize];
  env.shared_bytes_index = 0;
}

fn host_read_buffer<TEnvironment: Environment>(mut env: FunctionEnvMut<ImportObjectEnvironment<TEnvironment>>, buffer_pointer: u32, length: u32) {
  let buffer_pointer: wasmer::WasmPtr<u32> = wasmer::WasmPtr::new(buffer_pointer);
  let env = env.data_mut();
  let store = env.store.lock();
  ...
  memory_view
    .read(
      ...
      &mut env.shared_bytes[env.shared_bytes_index..env.shared_bytes_index + length],
    )
    .unwrap();
  env.shared_bytes_index += length;
}

fn host_write_buffer<TEnvironment: Environment>(mut env: FunctionEnvMut<ImportObjectEnvironment<TEnvironment>>, buffer_pointer: u32, offset: u32, length: u32) {
  let buffer_pointer: wasmer::WasmPtr<u32> = wasmer::WasmPtr::new(buffer_pointer);
  let env = env.data_mut();
  let store = env.store.lock();
  let memory = env.memory.as_ref().unwrap();
  ...
  let length = length as usize;
  memory_view
    .write(buffer_pointer.offset() as u64, &env.shared_bytes[offset..offset + length])
    .unwrap();
}

fn host_take_override_config<TEnvironment: Environment>(mut env: FunctionEnvMut<ImportObjectEnvironment<TEnvironment>>) {
  let env = env.data_mut();
  let bytes = std::mem::take(&mut env.shared_bytes);
...
fn host_get_formatted_text<TEnvironment: Environment>(mut env: FunctionEnvMut<ImportObjectEnvironment<TEnvironment>>) -> u32 {
  ...
  env.shared_bytes = formatted_text.into_bytes();
  len as u32
}
