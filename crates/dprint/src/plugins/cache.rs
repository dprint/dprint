...
mod test {
  ...
  use crate::plugins::PluginSourceReference;
  use crate::plugins::implementations::WASM_CACHE_VERSION;
  ...
  async fn should_download_remote_file() -> Result<()> {
    ...
    let expected_file_path = PathBuf::from("/cache")
      .join("plugins")
      .join("test-plugin")
      .join(format!("0.2.0-{WASM_CACHE_VERSION}-aarch64"));
  ...
  async fn should_cache_local_file() -> Result<()> {
    ...
    let expected_file_path = PathBuf::from("/cache")
      .join("plugins")
      .join("test-plugin")
      .join(format!("0.2.0-{WASM_CACHE_VERSION}-x86_64"));
    ...
    let expected_file_path = PathBuf::from("/cache")
      .join("plugins")
      .join("test-plugin")
      .join(format!("0.1.0-{WASM_CACHE_VERSION}-x86_64"));
  ...
  async fn forget_removes_npm_extract_dir() -> Result<()> {
    ...
    let compiled_wasm_path = environment
      ...
      .join("plugins")
      .join("test-plugin")
      .join(format!("1.0.0-{WASM_CACHE_VERSION}-x86_64"));
