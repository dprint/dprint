...
```jsonc
{
  "plugins": [
    ...
    "npm:@dprint/plugins@1.0.0/json/plugin.wasm",
...
- When a plugin is published to npm, `dprint init`, `dprint add` and `dprint config update` write the `npm:` form for it instead of a `plugins.dprint.dev` URL, and `dprint config update` moves an existing URL entry over to the package. The version always comes from the npm registry. Only an entry pointing at `plugins.dprint.dev` moves — a self-hosted copy of a plugin, or one referenced by a local path, is left as you wrote it.
